...
} from "@/redux/apiSlices/articleApi";
import Swal from "sweetalert2";
import { uploadVideoChunks } from "@/util/uploadVideoChunks";
...
const ArticleModal: React.FC<IArticleProps> = ({
  open,
  setOpen,
  setValue,
  refetch,
  ...
  console.log("video rul", videoUrl);

  const [form] = Form.useForm();
  console.log("Upload progrss", uploadProgress);
    ...
    <Modal
      ...
      title={
        <p className="text-xl text-white mt-1">
          {value?._id ? "Edit Article" : "Create An Article"}
        </p>
      }
      open={open}
      onOk={hideModal}
      ...
      >
        ...
        >
          ...
          <div
            ...
            style={{ height: 48, border: "1px solid #C3C4C6", borderRadius: 8 }}
          >
            <label
              className=" cursor-pointer border bg-[#C3C4C6] w-[170px] p-1 h-[34px] flex items-center gap-2 border-[#C3C4C6] rounded-md"
              htmlFor="image"
            >
              {image?.name ? (
                <>{image?.name}</>
        ...
        >
          ...
          >
            {programs?.data?.map((program: any, index: number) => {
              return (
                <Select.Option key={index} value={program?._id}>
                  {program?.title}
                </Select.Option>
              );
            })}
